...
class VaultCopyModule(ObsidianHtmlModule):
    ...
    def run(self):
        ...
        if self.persistent is not True:
            raise Exception(
                f"Module of type {self.module_class_name} needs to be created persistently, otherwise the temporary directory will be deleted immediately, instead of at exit."
            )
        ...
        self.copy_vault_to_tmpdir()

    def create_temporary_directory(self):
        # Create temp dir
        tmpdir = tempfile.TemporaryDirectory()

        # Store so that the reference will be kept, preventing automatic cleanup of the tmpdir
        # Note that this also requires the module itself to be kept, hence the persistance requirement.
        self.store("tmpdir", tmpdir)
        return tmpdir
